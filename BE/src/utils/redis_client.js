...
const ENABLE_REDIS = String(process.env.ENABLE_REDIS || '').trim().toLowerCase() === 'true';

if (!ENABLE_REDIS) {
  console.log('Redis client disabled by feature toggle (ENABLE_REDIS=false)');
  ...
  module.exports = {
    ...
    async *scanIterator() {
      // No-op iterator when Redis is disabled.
    },
    async del() {
      return false;
    },
  ...
  const enhancedRedisClient = {
    ...
    async del(key) {
      try {
        ...
        if (!key || typeof key !== 'string' || key.trim() === '') {
          console.error(`Attempted to delete with invalid key: "${key}"`);
          return false;
        }
    ...
    async setEx(key, ttl, value) {
      try {
        if (!redisClient.isReady) {
          console.log('Redis not ready when attempting to set', key);
          return false;
        }
          ...
          try {
            ...
            if (value.includes('"data"')) {
              ...
              if (
                ...
                console.error('Attempting to cache invalid data structure:', parsed);
                return false;
              }
            }
          } catch (parseErr) {
            console.error('Invalid JSON being cached:', parseErr);
            return false;
          }
        } else {
          console.error('Non-string value being cached');
          return false;
        }

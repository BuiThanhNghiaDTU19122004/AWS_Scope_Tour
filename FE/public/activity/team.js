...
//   }
    async function fetchTeams(page = 1) {
        let url = `${API_BASE_URL}/team?created_by=${user.user_id}&page=${page}&limit=${teamsPerPage}`;
  ...
  async function searchTeams(searchQuery) {
      ...
      try {
          const url = `${API_BASE_URL}/team?search=${encodeURIComponent(searchQuery.trim())}&created_by=${user.user_id}`;
          const response = await fetch(url);
          if (!response.ok) throw new Error('Network response was not ok');
          const data = await response.json();
